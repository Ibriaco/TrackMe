...
The architecture of our system can be streamlined into 3 logic layers:
	...
	\subsection{Component View}
		...
		\subsection{Other Design Decisions}
			...
			Furthermore, native applications guarantee their optimal integration the one with the other and with the entire ecosystem (referring for example to an Apple Watch paired to an iPhone device).
	...
	\section{Implementation, Integration and Test Plan}
		...
		In the following sections we are going to illustrate how external components will be integrated with the ones implemented for the system and what testing strategy will be adopted.
			...
			The integration of components and its testing should be carried out as soon as possible, although some conditions must be respected. In the very first place, external components and API must be available and serviceable even before the beginning of the implementation process. Secondly, the modules which are being integrated in the system are supposed to have at least the operations concerning on another created, if not carried out completely. The developed operations should, in the first place, pass unit tests to make sure that the components are working properly, and in case of failure, it will be guaranteed that a misfunctioning is located in the integration process itself and it will be consequently much easier to target the problem and solve it.\\
			More clearly, 
	...
	\begin{itemize}
		...
		\begin{center}
			\begin{tabular}{| c | c | c |}
				...
				03/12/2018 & Requirements Traceability & 1 \\ \hline
				Total &									& \\ \hline
			\end{tabular}
		\end{center}
